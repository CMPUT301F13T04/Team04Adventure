...
/**
 * StorageManager is meant to write data into the SQLite database. This data is
 * meant for access in offline mode in the case that the user chooses offline
 * mode or to view stories that the user has cached in the device memory.
 * 
...
public class StorageManager implements Storage {
	...
	/**
	 * Constructor that also creates an instance of SQLiteHelper for easier DB
	 * access.
	 * 
	 * @param context
	 *            current context.
	...
	public boolean checkStory(String id) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_STORIES, sId, swhere,
	...
	/**
	 * Adds a Story to the DB from information contained in the object story.
	 * 
	 * @param story
	 *            story to be added.
	 */
	public void addStory(Story story) {
		...
		database.insert(SQLiteHelper.TABLE_STORIES, null, values);
		...
		if (!story.getFrags().isEmpty()) {
			for (Frag f : story.getFrags()) {
				...
				database.insert(SQLiteHelper.TABLE_STORY_FRAGS, null, values);
			}
		}
	...
	private void addFrags(Frag f) {
		...
		database.insert(SQLiteHelper.TABLE_FRAGS, null, values);
		...
		if (!f.getAnnotations().isEmpty()) {
			for (Annotation a : f.getAnnotations()) {
				...
				database.insert(SQLiteHelper.TABLE_FRAGS_ANNOTS, null, values);
			}
		}
		...
		if (!f.getChoices().isEmpty()) {
			for (Choice c : f.getChoices()) {
				...
				database.insert(SQLiteHelper.TABLE_FRAGS_CHOICE, null, values);
			}
		}
		...
		if (!f.getPictures().isEmpty()) {
			for (Media m : f.getPictures()) {
				...
				database.insert(SQLiteHelper.TABLE_FRAGS_MEDIA, null, values);
	...
	private long addMedia(Media m) {
		...
		database.insert(SQLiteHelper.TABLE_MEDIA, null, values);
	...
	private long addChoices(Choice c) {
		...
		database.insert(SQLiteHelper.TABLE_CHOICE, null, values);
		...
		if (cursor != null && cursor.moveToFirst()) {
			lastId = cursor.getLong(0); // The 0 is the column index, we only
	...
	private long addAnnots(Annotation a) {
		...
		database.insert(SQLiteHelper.TABLE_ANNOTS, null, values);
	...
	private void deleteFragment(Frag f) {
		...
		database.delete(SQLiteHelper.TABLE_FRAGS_CHOICE,
				SQLiteHelper.COLUMN_FID + " = ?", fragargs);

		database.delete(SQLiteHelper.TABLE_FRAGS_ANNOTS,
				SQLiteHelper.COLUMN_FID + " = ?", fragargs);
	...
	public ArrayList<Story> getAll() {
		...
		this.open();
		Cursor cursor = database.query(SQLiteHelper.TABLE_STORIES, allStories,
	...
	/**
	 * Returns a single story object of the story with id sid.
	 * 
	 * @param sid
	 *            ID of the story to get.
	...
	/**
	 * Returns a single story object stored in the story object passed in.
	 * 
	 * @param story
	 *            story to get info from.
	 * @return a Story to be displayed to the user.
	 */
	private Story getInfo(Story story) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_STORY_FRAGS, fIds,
				sFwhere, whereargs, null, null, null);
	...
	public Frag getFrag(String fid) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_FRAGS, allFrags,
				fwhere, whereargs, null, null, null);
	 ...
	 * 
	 * @param fid
	 *            the fragment id to get the annotation of.
	...
	private ArrayList<Annotation> getFragAnnotInfo(String fid) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_FRAGS_ANNOTS, aIds,
				awhere, whereargs, null, null, null);
	...
	/**
	 * Returns an ArrayList of Videos or Pictures depending on the parameter
	 * type.
	 * 
	 * @param fid
	 *            ID of the media object to get info from.
	 * @param type
	...
	private ArrayList<Media> getFragMediaInfo(String fid, String type) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_FRAGS_MEDIA, mIds,
				mwhere, whereargs, null, null, null);
	...
	/**
	 * Returns a single Picture or Video object depending on the value in type.
	 * 
	 * @param mid
	 * @param type
	...
	private Media getMedia(long mid, String type) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_MEDIA, allMedia,
				fwhere, whereargs, null, null, null);
	...
	/**
	 * Helper method that stores all the Media attributes in a new Media object
	 * for addition to an ArrayList.
	 * 
	 * @param cursor
	...
	/**
	 * Returns an ArrayList of Choices that belong to the fragment with ID of
	 * fid.
	 * 
	 * @param fid
	 *            ID of the fragment to get info from.
	...
	private ArrayList<Choice> getFragChoiceInfo(String fid) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_FRAGS_CHOICE, cIds,
				cwhere, whereargs, null, null, null);
	...
	/**
	 * Returns a single Choice object with ID of cid.
	 * 
	 * @param cid
	 *            ID of the choice.
	...
	private Choice getChoice(long cid) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_CHOICE, allChoice,
				fwhere, whereargs, null, null, null);
	...
	/**
	 * Returns a single Annotation object with ID of aid.
	 * 
	 * @param aid
	 *            ID of the choice.
	...
	private Annotation getAnnotation(long aid) {
		...
		Cursor cursor = database.query(SQLiteHelper.TABLE_ANNOTS, allAnnots,
				fwhere, whereargs, null, null, null);
	...
	/**
	 * Helper method that stores all the Annotation attributes in a new
	 * Annotation object for addition to an ArrayList.
	 * 
	 * @param cursor
	...
	/**
	 * Helper method that stores all the Choice attributes in a new Choice
	 * object for addition to an ArrayList.
	 * 
	 * @param cursor
	...
	/**
	 * Helper method that stores all the Fragment attributes in a new Fragment
	 * object for addition to an ArrayList.
	 * 
	 * @param cursor
	...
	/**
	 * Helper method that stores all the Story attributes in a new Story object
	 * for addition to an ArrayList.
	 * 
	 * @param cursor
